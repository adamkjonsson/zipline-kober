...
| [`REPETITION-PHASE-PLAN.md`](REPETITION-PHASE-PLAN.md) | A way for a spec to ask a question about a repeated field, so HTTP can choose its own body framing instead of assuming one. | **In progress** — `examples/http.yaml` frames its own body; fuzzing and documentation (Stages 7–8) open. |
