...
| [`REPETITION-PHASE-PLAN.md`](REPETITION-PHASE-PLAN.md) | A way for a spec to ask a question about a repeated field, so HTTP can choose its own body framing instead of assuming one. | **In progress** — the construct, both implementations and the corpus are done; documentation (Stage 8) open. |
