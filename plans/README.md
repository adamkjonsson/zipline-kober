...
| [`TRANSFORM-PHASE-PLAN.md`](TRANSFORM-PHASE-PLAN.md) | Byte transforms — decompression and decryption: a `transform` construct over already-decoded bytes with an optional in-place sub-decode, `concat` for chunked input, a declared-in-spec / registry-supplied split that keeps `check` static, and `params_digest` for keys. | **In progress** — Stage 0 and the Stage 1 spike done; Stages 1b (#49, phantom messages after a gap) and 1c (#50, `startswith`/`endswith`) done; the spike's three decisions made; Stage 2 next. |
