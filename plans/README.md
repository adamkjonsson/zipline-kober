...
| [`VERDICT-PHASE-PLAN.md`](VERDICT-PHASE-PLAN.md) | The `0.4.0` milestone: stream confirmation (a stream that fails before its first good message is declined, not retried), the entry unit's `emit` honoured by both backends, `undecodable` rather than `truncated` for a field starved under `check=False`, and the deeper pipeline as a checked-in script. | **Planned** — one judgment call (§3.3a) open. |
| [`TRANSFORM-PHASE-PLAN.md`](TRANSFORM-PHASE-PLAN.md) | Byte transforms — decompression and decryption: a `transform` construct over already-decoded bytes with an optional in-place sub-decode, `concat` for chunked input, a declared-in-spec / registry-supplied split that keeps `check` static, and `params_digest` for keys. | **Planned** — nine design questions with leanings, to be settled by a Stage 1 spike. |
